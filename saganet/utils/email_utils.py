...
from saganet.utils.timezone import my_timezone
        ...
        if self.enable:
            ...
            try:
                return requests.post(
                    f"https://api.mailgun.net/v3/{_source}/messages",
                    auth=("api", self.MAILGUN_API_KEY),
                    data={
                        "from": f"<agent name>🤖 <mailgun@{_source}>",
                        "to": [f"{_target}"],
                        "subject": f"[{self.exp_id}] {subject}",
                        "text": (
                            "\n\n"
                            + content
                            + "\n\n<your sign off>\n"
