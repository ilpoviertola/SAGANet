...
from saganet.ext.rotary_embeddings import compute_rope_rotations
from saganet.model.embeddings import TimestepEmbedder
...
class MMAudio(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if latent_mean is None:
            ...
            assert (
                latent_mean.numel() == latent_dim
            ), f"{latent_mean.numel()=} != {latent_dim=}"
    ...
    def preprocess_conditions(
        ...
        """
        assert (
            clip_f.shape[1] == self._clip_seq_len
        ), f"{clip_f.shape=} {self._clip_seq_len=}"
        assert (
            sync_f.shape[1] == self._sync_seq_len
        ), f"{sync_f.shape=} {self._sync_seq_len=}"
        assert (
            text_f.shape[1] == self._text_seq_len
    ...
    def predict_flow(
        ...
        """
        assert (
            latent.shape[1] == self._latent_seq_len
    ...
    def load_weights(self, src_dict) -> None:
        ...
        self.load_state_dict(src_dict, strict=(not self.use_lora))
...
def small_44k_sa(**kwargs) -> MMAudio:
    num_heads = 7
    return MMAudio(
        latent_dim=40,
        clip_dim=1024,
...
def medium_44k_sa(**kwargs) -> MMAudio:
    num_heads = 14
    return MMAudio(
        latent_dim=40,
        clip_dim=1024,
...
def get_my_mmaudio(name: str, **kwargs) -> MMAudio:
    if name == "small_44k_sa":
        kwargs["use_lora"] = True
        return small_44k_sa(**kwargs)
    if name == "small_44k_sa_no_lora":
        kwargs["use_lora"] = False
        return small_44k_sa(**kwargs)
    if name == "medium_44k_sa":
        kwargs["use_lora"] = True
        return medium_44k_sa(**kwargs)
    if name == "medium_44k_sa_no_lora":
        kwargs["use_lora"] = False
        return medium_44k_sa(**kwargs)
...
if __name__ == "__main__":
    network = get_my_mmaudio("small_44k_sa")

    # print the number of parameters in terms of millions
    num_params = sum(p.numel() for p in network.parameters()) / 1e6
    print(f"Number of parameters: {num_params:.2f}M")

    network_lora = get_my_mmaudio("small_44k_sa", use_lora=True)
    num_params_lora = sum(p.numel() for p in network_lora.parameters()) / 1e6
    print(f"Number of parameters with LoRA: {num_params_lora:.2f}M")

    B = 1
    net_to_use = network_lora.to("cuda")
    dummy_sync = torch.randn(B, 56, 768).to(net_to_use.device)
    dummy_clip = torch.randn(B, 40, 1024).to(net_to_use.device)
    dummy_text = torch.randn(B, 77, 1024).to(net_to_use.device)
    dummy_latent = torch.randn(B, 216, 40).to(net_to_use.device)
    dummy_t = torch.randn(B).to(net_to_use.device)
    net_to_use(
        dummy_latent,
        dummy_clip,
        dummy_sync,
