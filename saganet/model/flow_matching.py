...
class FlowMatching:
    ...
    def __init__(
        self, min_sigma: float = 0.0, inference_mode="euler", num_steps: int = 25
    ):
    ...
    def loss(
        self, predicted_v: torch.Tensor, x0: torch.Tensor, x1: torch.Tensor
    ) -> torch.Tensor:
    ...
    def run_t0_to_t1(
        ...
        if self.inference_mode == "adaptive":
            return odeint(
                fn, x0, torch.tensor([t0, t1], device=x0.device, dtype=x0.dtype)
            )
