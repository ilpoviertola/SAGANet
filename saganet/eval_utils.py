...
from saganet.data.av_utils import (
    ImageInfo,
    VideoInfo,
    read_frames,
    reencode_with_audio,
...
from saganet.model.sequence_config import (
    CONFIG_44K_SA,
    SequenceConfig,
)
...
saganet_small = ModelConfig(
    model_name="small_44k_sa",
    model_path=Path("./weights/saganet.pth"),
    vae_path=Path("./ext_weights/v1-44.pth"),
    mode="44k_sa",
    bigvgan_16k_path=None,
)

saganet_small_no_lora = ModelConfig(
    model_name="small_44k_sa_no_lora",
    model_path=Path("./weights/saganet_no_lora.pth"),
    vae_path=Path("./ext_weights/v1-44.pth"),
    mode="44k_sa",
    bigvgan_16k_path=None,
)
...
all_model_cfg: dict[str, ModelConfig] = {
    "saganet_small": saganet_small,
    "saganet_small_no_lora": saganet_small_no_lora,
}
...
def generate(
    ...
    sync_detail_video: Optional[torch.Tensor],
    text: Optional[list[str]],
    mask_video: Optional[torch.Tensor],
    mask_detail_video: Optional[torch.Tensor],
    *,
    ...
    if clip_video is not None:
        ...
        clip_features = feature_utils.encode_video_with_clip(
            clip_video, batch_size=bs * clip_batch_size_multiplier
        )
        if image_input:
    ...
    if sync_video is not None and not image_input:
        assert sync_detail_video is not None, "Sync detail video must be provided"
        assert mask_video is not None, "Mask video must be provided"
        assert mask_detail_video is not None, "Mask detail video must be provided"
        sync_video = sync_video.to(device, dtype, non_blocking=True)
        sync_detail_video = sync_detail_video.to(device, dtype, non_blocking=True)
        mask_video = mask_video.to(device, dtype, non_blocking=True)
        mask_detail_video = mask_detail_video.to(device, dtype, non_blocking=True)
        sync_features = feature_utils.encode_video_with_sync(
            sync_video,
            sync_detail_video,
            mask_video,
            mask_detail_video,
            batch_size=bs * sync_batch_size_multiplier,
        )
    else:
    ...
    empty_conditions = net.get_empty_conditions(
        ...
        negative_text_features=(
            negative_text_features if negative_text is not None else None
        ),
    )

    cfg_ode_wrapper = lambda t, x: net.ode_wrapper(
        t, x, preprocessed_conditions, empty_conditions, cfg_strength
    )
...
def load_mask_video(
    ...
    output_frames, all_frames, orig_fps = read_frames(
        video_path,
        list_of_fps=[_SYNC_FPS],
        start_sec=0,
        end_sec=duration_sec,
        need_all_frames=load_all_frames,
        v_format=v_format,
    )
    mask_chunk = output_frames[0]
    mask_chunk = torch.from_numpy(mask_chunk)
    mask_frames = mask_transform(mask_chunk)
    mask_length_sec = mask_frames.shape[0] / _SYNC_FPS
    if mask_length_sec < duration_sec:
        log.warning(
            f"Mask video is too short: {mask_length_sec:.2f} < {duration_sec:.2f}"
        )
        log.warning(f"Truncating to {mask_length_sec:.2f} sec")
        duration_sec = mask_length_sec
    mask_frames = mask_frames[: int(_SYNC_FPS * duration_sec)]
    video_info = VideoInfo(
        duration_sec=duration_sec,
        fps=orig_fps,
        clip_frames=None,  # No clip frames for mask video
        sync_frames=mask_frames,
