...
from saganet.data.data_setup import setup_test_datasets
from saganet.runner import Runner
from saganet.utils.dist_utils import info_if_rank_zero
...
def sample(cfg: DictConfig):
    ...
    if dataset.__class__.__name__ == "ExtractedAVS":
        data_cfg = cfg.data.ExtractedAVS_test
        cfg["video_joiner_root"] = cfg.data.AVS.root
    elif dataset.__class__.__name__ == "MusicRaw":
        cfg["video_joiner_root"] = cfg.data.RawMusic.root
        data_cfg = cfg.data.RawMusic_test
    elif dataset.__class__.__name__ == "MusicHybrid":
        cfg["video_joiner_root"] = cfg.data.HybridMusic.root
        data_cfg = cfg.data.HybridMusic_test
    if dataset.__class__.__name__ == "URMPDataset":
        cfg["video_joiner_root"] = cfg.data.URMP.root
        data_cfg = cfg.data.URMP
    else:
    ...
    if cfg["weights"] is not None:
        ...
        if weights is not None:
            ...
            runner.load_weights(weights, "network")
