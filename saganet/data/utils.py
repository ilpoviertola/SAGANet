...
scratch_path = Path(
    os.environ["SLURM_SCRATCH"] if "SLURM_SCRATCH" in os.environ else "/dev/shm"
)
...
def load_shards_and_share(
    data_path: Union[str, Path], ids: list[int], in_memory: bool
) -> MemoryMappedTensor:
    if local_rank == 0:
        with tempfile.NamedTemporaryFile(
            prefix="shared-tensor-", dir=get_tmp_dir(in_memory)
        ) as f:
...
def load_shards(
    ...
    *,
    ...
    mm_tensor = MemoryMappedTensor.empty(
        shape=(len(ids), *first_item.shape),
        dtype=torch.float32,
...
def share_tensor_to_all(x: Optional[MemoryMappedTensor]) -> MemoryMappedTensor:
    ...
    if local_rank == 0:
        ...
        data = MemoryMappedTensor.from_filename(
            filename=filename, dtype=data_type, shape=data_shape
        )
